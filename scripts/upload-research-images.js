...
// Upload research images to R2, generate 480px-wide JPEG thumbnails, and update research.csv.
...
const CSV_PATH = path.join(ROOT, "googleform", "research.csv")
const IMAGES_DIR = path.join(ROOT, "googleform", "research")
...
const tmpDir = path.join(os.tmpdir(), "sit-design-expo2026-research")
...
const main = async () => {
  ...
  for (const row of records) {
    ...
    const originalKey = `research/images/${filename}`
    const thumbName = `${baseName}_thumb.jpg`
    const thumbKey = `research/images/${thumbName}`
    ...
    row.image_url = `${BUCKET_ENDPOINT}/research/images/${filename}`
    row.image_thumb_url = `${BUCKET_ENDPOINT}/research/images/${thumbName}`
    updated += 1
  }
